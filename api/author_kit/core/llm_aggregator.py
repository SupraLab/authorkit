...
class LLMProviderBase(ABC):
    ...
    def __init__(self, config: dict[str, Any] = None, aggregator=None):
        ...
        self.aggregator = aggregator  # Reference to ProviderRegistry for model list cache
    ...
    @abstractmethod
    def get_llm_instance(self, overrides) -> LLM | BaseChatModel:
            ...
            self.cached_models = [
                {
                    ...
                    "name": model.get(
                        "name", model.get("display_name", model.get(self.model_key, ""))
                    ),
    ...
    def get_model_endpoint(self, overrides=None) -> str:
        ...
        url = (
            overrides
            and overrides.get("endpoint")
            or self.config.get("endpoint", self.get_base_url())
        )
...
class AnthropicProvider(LLMProviderBase):
    ...
    def _do_models_request(self, url: str, headers: dict[str, str] = None) -> list[str]:
        """Send a request to the provider to fetch available models."""
        default_headers = {"x-api-key": self.get_api_key(), "anthropic-version": "2023-06-01"}
        if headers:
...
class LMStudioProvider(LLMProviderBase):
    ...
    def get_llm_instance(self, overrides) -> BaseChatModel:
        return ChatOpenAI(
            api_key="not-needed",
            base_url=overrides.get("endpoint", self.get_base_url()),
            model=overrides.get("model", self.get_current_model() or "local-model"),
            temperature=overrides.get(
                "temperature", self.config.get("temperature", DEFAULT_TEMPERATURE)
            ),
            max_tokens=overrides.get(
...
class CustomProvider(LLMProviderBase):
    ...
    def get_llm_instance(self, overrides) -> BaseChatModel:
        ...
        return ChatOpenAI(
            ...
            api_key=self.get_api_key(),
            model=self.get_current_model() or "custom-model",
            temperature=overrides.get(
                "temperature", self.config.get("temperature", DEFAULT_TEMPERATURE)
            ),
            max_tokens=overrides.get(
...
class ProviderRegistry:
    """Registry for LLM providers; uses injected SettingsStore (no global settings)."""
    ...
    def get_provider(self, provider_name: str) -> LLMProviderBase | None:
        """Get a fresh provider instance (no stale cache when API keys change)."""
        ...
        if not provider_class:
            return None
        return provider_class(config, aggregator=self)

    def get_active_llms(self) -> list[str]:
        """Returns all configured LLM profile names."""
        return list(self._settings_store.get_llm_configs().keys())

    def _get_provider_config(self, provider_name: str) -> dict[str, Any] | None:
        settings = self._settings_store.get_llm_configs()
        if not settings:
            return None
        return settings.get(provider_name)

    def _get_provider_class(self, provider_name: str) -> type[LLMProviderBase] | None:
    ...
    def get_cached_models(self, provider_name: str) -> list[dict[str, Any]] | None:
...
class LLMAPIAggregator:
    """LLM calls with streaming; uses SettingsStore for active provider and keys."""

    def __init__(self, settings_store: SettingsStore):
        self._settings_store = settings_store
        self.aggregator = ProviderRegistry(settings_store)
    ...
    def send_prompt_to_llm(
        ...
        provider_name = overrides.get("provider") or self._settings_store.get_active_llm_name()
        if provider_name in ["Local", "Default"]:
            provider_name = self._settings_store.get_active_llm_name()
            overrides = {}
    ...
    def stream_prompt_to_llm(
        ...
        logging.debug(
            f"Starting stream_prompt_to_llm, interrupt_flag: {self.interrupt_flag.is_set()}"
        )
        overrides = overrides or {}
        ...
        try:
            ...
            logging.debug("Stream cleanup, setting is_streaming=False, clearing interrupt_flag")
            self.is_streaming = False
    ...
    def interrupt(self):
        ...
        if self.is_streaming:
            logging.debug("Interrupting stream, setting interrupt_flag")
