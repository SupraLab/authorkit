...
class SelectionAttachment(BaseModel):
    ...
    compendium_category: str | None = None
    compendium_name: str | None = None
    range_label: str | None = Field(
        None,
...
class ChatMessage(BaseModel):
    role: str
    content: str
    context: WorkshopMessageContext | None = Field(
        None,
...
class WorkshopThreadCreate(BaseModel):
    workspace_root: str
    title: str | None = None
    seed_type: str | None = Field(None, description="scene | character | world | none")
    seed_ref: str | None = Field(None, description="Scene UUID or compendium key")
...
class WorkshopThreadUpdate(BaseModel):
    title: str = Field(
        ..., min_length=1, max_length=240, description="New display name for the thread"
    )
